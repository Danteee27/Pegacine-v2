...
function Player() {
  ...
  useEffect(() => {
    async function fetchWatchingMovies() {
      const request = await axiosInstance3.get(
        `http://localhost:3000/api/user/profiles/watching?profile_id=${userDetails?.id}&page=1&pageSize=999`,
      );
  ...
  useEffect(() => {
    const matchingMovieArr = watchingMovies.filter(
      (movie) => movie?.movie_id === movieData?.movie_id,
    );
    if (matchingMovieArr.length > 0) {
      video.current.currentTime +=
        (isFinite(matchingMovieArr[0]?.stoppedAt)
          ? matchingMovieArr[0]?.stoppedAt
          : 0) * 60 || 0;
    }
  }, [watchingMovies]);
  ...
  useEffect(() => {
    ...
    const updateTime = () => {
      if (watchBar.current) {
        ...
        let minutes = Math.floor(totalSecondsRemaining / 60 - hours * 60)
          .toString()
          .padStart(2, '0');
        let seconds = Math.floor(
          ...
          .toString()
          .padStart(2, '0');
        timeRemaining.current.textContent = `${
          hours ? hours + ':' : ''
        }${minutes}:${seconds}`;
        ...
        const updateWatchingTime = async () => {
          if (Math.floor(video.current.currentTime / 60) >= 2) {
            const request = await axiosInstance3.post(
              `http://localhost:3000/api/user/profiles/watching?profile_id=${
                userDetails?.id
              }&movie_id=${movieData?.movie_id}&stoppedAt=${Math.floor(
                video.current.currentTime / 60,
              )}`,
            );
            ...
            const request = await axiosInstance3.post(
              `http://localhost:3000/api/user/profiles/watching?profile_id=${
                userDetails?.id
              }&movie_id=${movieData?.movie_id}&stoppedAt=${9999999}`,
            );
          }
        };
    ...
    progressBar.current.addEventListener('click', (e) => {
      const pos =
        (e.pageX -
          progressBar.current.offsetLeft -
          progressBar.current.offsetParent.offsetLeft) /
        progressBar.current.offsetWidth;
      if (isFinite(video.current.duration) && isFinite(video.current.duration))
        video.current.currentTime =
          (isFinite(pos) ? pos : 0) *
          (isFinite(video.current.duration) ? video.current.duration : 0);
        ...
        <div className={'videoPlayer videoContainer'} ref={videoContainer}>
          ...
          <div className={'controlsContainer'} ref={controlsContainer}>
            ...
            <div className={'controls'}>
              ...
              <button
                className={'next'}
                onMouseEnter={() => setIsNextEpisodeHover(true)}
              ...
              <button
                className={'episode'}
                onMouseEnter={() => setIsEpisodeHover(true)}
          ...
          {isEpisodeHover && (
            <div
              className={'episodeHover'}
              onMouseEnter={() => setIsEpisodeHover(true)}
              onMouseLeave={handleOnMouseLeaveEpisode}
            >
              <div className={'filmTitle'}>
                {isSeries ? series.seriesName : movieData.title}
              </div>
              <div className={'episodeList'}>
                {isSeries &&
                  series?.movies &&
                  series.movies.map((movie, index) => (
                    ...
                    >
                      <div className={'episodeRow'}>
                        <div className={'headerEpisodeRow'}>
                          <div className={'orderAndTitleEpisode'}>
                            <div className={'orderEpisode'}>
                              {movie.seriesOrder}
                            </div>
                            <div className={'titleEpisode'}>{movie.title}</div>
                          </div>
                          <div className={'progressBarForEpisode'}>
                            {/* ProgressBar */}
                          </div>
                        </div>
                        <div className={'contentEpisodeRow'}>
                          ...
                          <div className={'descriptionEpisode'}>
                            {movie.overview}
                          </div>
                        </div>
                      </div>
