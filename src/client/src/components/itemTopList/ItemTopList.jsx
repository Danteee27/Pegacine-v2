...
export default function ItemTopList({ index, top = index + 1, movieData }) {
  ...
  return (
    <div style={{ width: 225 }}>
      <div
        ...
        onMouseEnter={() => {
          if (indexSvg.current) {
            indexSvg.current.style.opacity = 0;
            indexSvg.current.style.width = 0;
          }
          if (poster.current) {
            poster.current.style.width = '100%';
          }
          setIsHovered(true);
        }}
        onMouseLeave={() => {
          if (indexSvg.current) {
            indexSvg.current.style.opacity = 1;
            indexSvg.current.style.width = '30%';
          }
          if (poster.current) {
            poster.current.style.width = '50%';
          }
          setIsHovered(false);
        }}
      >
            ...
            <div className="detail-info">
              ...
              <div className="addtional-field">
                <span>
                  ...
                  {cast &&
                    cast.map((caster) => (
                      <span>{caster.character_name}, </span>
                    ))}
                </span>
                <span>
